...
<title>Insert title here</title>
    ...
    class PeriodMount 
    {
        private $data_period_no = 0;
        private $data_start_date;// = date_create();    // 贷款首期借款日期
        private $data_due_date;// = date_create();      // 贷款本期还款日期
        private $data_principal = 0;                    // 本期总欠本金
        private $data_due_principal = 0;                // 本期应还本金
        private $data_due_days = 0;                     // 本期天数
        private $data_due_interest_real = 0.0;          // 本期应还利息_原始小数
        private $data_due_interest = 0;                 // 本期应还利息_取整
        private $data_due_total = 0;                    // 本期应还本息
        private $data_z_1_B = 1;                    // 本期本息率 = 1 + 年率 /360* 本期天数
        private $data_z_pai = 1;                    // 连乘积
        
        function __construct()
        {
            $this->data_start_date = date_create(); 
            $this->data_due_date = date_create();
        ...
        { // 设置本期总欠款本金
            $this->data_principal = $principal;
        }
        
        public function getNextPeriodPrincipal()
        { // 计算下期总欠款本金
            return $this->data_principal - $this->data_due_principal;
        }
        
        public function cal_principal_interest($new_principal,$rate,$per_Period_round)
        { // 计算本期应还本金、利息、剩余本金。
            $this->data_principal = $new_principal;									// 本期总欠本金
            ...
            $this->data_due_principal = $per_Period_round - $new_interest_round;
            
            if ( $this->data_due_principal > $this->data_principal ) // 修正本期应还本金，如果应还本金，大于剩余本金，就是错误，改为剩余本金。
            	$this->data_due_principal = $this->data_principal;
            
            $this->data_due_total = $this->data_due_principal + $this->data_due_interest;
        }
        
        public function cal_last_due_principal()
        { // 修正最后一期应还本金，如果没还完本金，全部归还。
        	if ( $this->data_due_principal < $this->data_principal )
        		$this->data_due_principal = $this->data_principal;
        	$this->data_due_total = $this->data_due_principal + $this->data_due_interest;
        }
        ...
        {
            $this->data_period_no = $x;
            $i = new DateInterval("P".$x."M");
            
          
            $this->data_start_date= date_create_from_format("Y-m-d H:i:s",date_format($start_date,"Y-m-d 00:00:00"));
            $this->data_due_date  = date_create_from_format("Y-m-d H:i:s",date_format($start_date,"Y-m-d 00:00:00"));
            
                  
            $this->data_due_date = date_add($this->data_due_date,$i);
        //    echo date_format($date,"Y/m/d")."<br>";
            $this->data_due_days = (int) date_diff($this->data_due_date,$this->data_start_date)->format("%a");
            return;
        }
        
        public function getDate()
        { // 获取本期还款日的一个副本
            $date= date_create_from_format("Y-m-d H:i:s",date_format($this->data_due_date,"Y-m-d 00:00:00"));
            return $date;            
        }
        
        public function fixDueDate($date)
        { // 计算本期还款日和某日期（上期还款日）间隔的天数
            $this->data_due_days = (int) date_diff($this->data_due_date,$date)->format("%a");
        }
    ...
    class TotalScedule
    {
        ...
        public function calPeriodMount()
        {
        	...
        	$this->PeriodMounts[$this->d4_total_Period+1] = new PeriodMount(); // 多生成一个，data_due_z_1_B = 1；
        	...
        	$this->d6_per_Period = $this->d1_all_loan * $first_z_pai / $sum_z_pai; // 求精确月供
        	$this->d6_per_Period_round = round( $this->d6_per_Period, 2, PHP_ROUND_HALF_UP ); // 求四舍五入到分月供
        	...
        	$this->PeriodMounts[$this->d4_total_Period]->cal_last_due_principal();
        ...
        public function echoTable()
        {
        	...
        	for ($x=0; $x <= $this->d4_total_Period+1+20; $x++) {
        		echo "    <tr>\n";
